...
def run_benchmark(exe_path, n0, n1, algorithm):
    print(f"Running RFFT throughput benchmark for {algorithm} from order {n0} to {n1}...")
...
def main():
    parser = argparse.ArgumentParser(description="Throughput Benchmark for RFFT")
    parser.add_argument("n0", type=int, help="Start FFT order (size 2^n)")
    parser.add_argument("n1", type=int, help="End FFT order (size 2^n)")
    parser.add_argument("algorithm", type=str, help="Algorithm to test (e.g., zldsp, fftw3, kfr, pffft)")
    ...
    try:
        exe_path = build_benchmark(args.algorithm, "throughput_rfft",
