...
int main(const int argc, char** argv) {
    ...
    std::array<F*, 2> out_aos_soa = {out_aos_soa_r.data(), out_aos_soa_i.data()};
    ...
    std::array<F*, 2> out_soa_soa = {out_soa_soa_r.data(), out_soa_soa_i.data()};

    std::vector<C, AlignedAllocator<C>> in_aos_copy = in_aos;
    cfft.forward(in_aos_copy.data(), out_aos_aos.data());

    in_aos_copy = in_aos;
    cfft.forward(in_aos_copy.data(), out_aos_soa);

    std::vector<F, AlignedAllocator<F>> in_soa_r_copy = in_soa_r;
    std::vector<F, AlignedAllocator<F>> in_soa_i_copy = in_soa_i;
    std::array<F*, 2> in_soa_copy = {in_soa_r_copy.data(), in_soa_i_copy.data()};
    cfft.forward(in_soa_copy, out_soa_aos.data());

    in_soa_r_copy = in_soa_r;
    in_soa_i_copy = in_soa_i;
    in_soa_copy = {in_soa_r_copy.data(), in_soa_i_copy.data()};
    ...
    std::array<F*, 2> back_aos_soa = {back_aos_soa_r.data(), back_aos_soa_i.data()};
    ...
    std::array<F*, 2> back_soa_soa = {back_soa_soa_r.data(), back_soa_soa_i.data()};

    in_aos_copy = in_aos;
    cfft.backward(in_aos_copy.data(), back_aos_aos.data());

    in_aos_copy = in_aos;
    cfft.backward(in_aos_copy.data(), back_aos_soa);

    in_soa_r_copy = in_soa_r;
    in_soa_i_copy = in_soa_i;
    in_soa_copy = {in_soa_r_copy.data(), in_soa_i_copy.data()};
    cfft.backward(in_soa_copy, back_soa_aos.data());
    ...
    cfft.backward(out_aos_aos_copy.data(), fwd_then_back.data());
